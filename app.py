...
_MAX_UPLOAD_BYTES = 20 * 1024 * 1024  # 20 MB ファイルアップロード上限
...
ユーザーが「{keyword}」で検索しました。以下の該当エントリから、
ユーザーが知りたそうな具体的な切り口を3〜4個提案してください。

## ルール
1. 該当エントリのタイトル・キーワード・要約から、テーマ別に分類する
2. 各提案は15文字以内の簡潔なラベル（例:「ST上昇の心電図所見」「不整脈の鑑別」）
3. **必ずJSON配列のみ**を返す。他のテキストは一切不要。
4. 例: ["ST上昇の心電図所見", "不整脈の鑑別", "治療方針"]
...
def download_thumbnail(_service, file_id: str, max_px: int = 200) -> bytes:
    """サムネイル用に軽量化した画像を返す（最大200px, JPEG 50%）。"""
...
def page_batch_analyze():
    ...
    mode = st.radio(
        ...
        ["新規解析", "一括再解析", "指示付き再解析", "患者データ編集",
         "レビュー", "一括レビュー済みに変更", "OCRテキスト抽出", "解析データの削除"],
        horizontal=True,
        key="batch_mode",
    )
    ...
    # =======================================================================
        ...
        st.subheader("📝 OCRテキスト抽出")
        st.caption("解析済み画像からテキスト情報を読み取り、全文検索を可能にします。")

        metadata = load_metadata()
        # OCR未実施の解析済み画像（患者データ除外）
        ocr_pending = [
            img for img in images
            if img["id"] in metadata
            and not is_patient_data(metadata[img["id"]])
        ...
        if not ocr_pending:
            ...
            _render_pagination_controls("batch_ocr_page", ocr_cur, ocr_total_pages, len(ocr_pending))
            ...
            for idx, img in enumerate(ocr_page_items):
                ...
                with cols[idx % 4]:
                    ...
                    st.checkbox(
                        ...
                        key=f"ocr_sel_{fid}",
                        value=st.session_state.get(f"ocr_sel_{fid}", False),
                    )

            st.markdown("---")
            if st.button("🚀 OCRテキスト抽出を実行", type="primary"):
                target = [img for img in ocr_pending if st.session_state.get(f"ocr_sel_{img['id']}")]
                if not target:
